...
- **角色化默认 + 白模为可选规避方案（v9.0 修正）**：本 skill 的最终目标是出**电影级武术打斗影像**——为了服务这个目标，**默认应根据用户需求（年龄 / 性别 / 气质 / 调性 / 武术体系 / 影视参考）适配具象角色形象**，让画面有真实人物的张力与情感。白模只在两种场景下使用：(1) 用户明确要做版权规避（如复刻《唐三》《王林》等 IP 桥段时，避免侵权）；(2) 用户明确要做动作 retarget 工作流（动作设计独立于最终角色）。其余情况直接根据用户需求想象适配的具象角色，不要硬套白模。
...
| 字段 | 说明 | 示例 |
|------|------|------|
| **呈现风格**（必填，默认白模 pre-vis） | 武指预演的视觉表达方式 | **3D 白模（默认）** / 线框模型 / 木质人偶 / 黑色剪影 / 半透明蓝图 / mocap 骨架点 / 灰模 / **角色化最终画面**（仅当用户明确要求时） |
| **体型与气质** | 性别气质、身高比例、体格倾向（白模也有体型） | 修长精瘦男性肩宽腿长 / 中等身材女性舞者身段 / 矮壮敦实大叔型 |
| **(可选) 角色化外貌** | 仅当呈现风格 = "角色化最终画面" 时填，白模模式跳过 | 中国女孩长相、约 22 岁、英气、五官立体 |
...
读取 `references/poster-template.md`，把 Step 1 的人物锚定卡 + Step 2 的招式列表填入主模板。
...
**输出物**：一段完整可直接复制的中文提示词。输出后告诉用户："这段直接复制到主流 AI 绘图工具，建议画面比例 1:1 或 4:3，分辨率尽量 ≥1024×1024。**这张多宫格海报既是你的武指工作单（整体动作字典），也是直接喂给视频模型的参考图——不需要再生成首帧图**。视频模型会根据 Step 4 视频 prompt 中的「参考图阅读协议」正确读图、按 16 格顺序连贯演完所有动作。如果之后想用自己上传的人物图替换白模，只需替换提示词中『演武者为同一位 ... 全程保持同一人物』这一段。"
...
- **类别 A 版权脱敏**：Step 3 图片 prompt **按工具严格度自行决定**（中文绘图工具对中文版权词较宽松，可保留部分增强辨识度；国际工具较严需脱敏）；**Step 4 视频 prompt 零容忍含专有名词**。
...
【A. 单格高清图提示词】（150-250 中文字符，纯文本，可直接喂 Seedream / 即梦 / Midjourney）

一段连贯的中文描述，融合：人物锚定卡完整描述（避免漂移）+ 该招的具体动作（朝向 / 重心 / 肢体）+ 镜头景别与角度 + 光线方向与色温 + 兵器状态（如有）+ 环境互动 + 风格关键词。
